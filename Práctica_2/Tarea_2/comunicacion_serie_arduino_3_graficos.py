...
ax1.grid(True)
ax1.set_ylabel("Ángulo (giroscopio)")

# subfigura 2: gx, gy, gz
line21, = ax2.plot(x, y[1,:], color='r')
ax2.legend(["ax"])
ax2.set_ylim([-180,180])
ax2.grid(True)
ax2.set_ylabel("Ángulo (acelerómetro)")

# subfigura 3: gx, gy, gz
line31, = ax3.plot(x, y[2,:], color='r')
ax3.legend(["theta"])
ax3.set_ylim([-180,180])
ax3.grid(True)
ax3.set_ylabel("Ángulo (complementario)")
...
with open("output"+timestr+".csv",'w') as file:
    ...
    try: 
        ...
        while True:
            ...
            data_bytes = ser.read(12) # payload
            ...
            datareal_gx  = struct.unpack('<f',data_bytes[4:8])[0] # gx
            datareal_theta  = struct.unpack('<f',data_bytes[8:12])[0] # gx
            ...
            y[:,-1] = [datareal_ax,datareal_gx,datareal_theta]
            # actualizamos los plots
            line11.set_xdata(x)
            line11.set_ydata(y[0,:])
            line21.set_xdata(x)
            line21.set_ydata(y[1,:])
            line31.set_xdata(x)
            line31.set_ydata(y[2,:])
            ...
            #writer.writerow(y[:,-1])
            writer.writerow([datareal_ax, datareal_gx, datareal_theta])
