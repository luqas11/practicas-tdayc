...
line12, = ax1.plot(x, y[1,:], color='g', linewidth=0.6) # ay
line13, = ax1.plot(x, y[2,:], color='b', linewidth=0.6) # az
ax1.legend(["gx","ax","theta"])
ax1.set_ylim([-8,8])
ax1.grid(True)
...
with open("output"+timestr+".csv",'w') as file:
    ...
    try: 
        ...
        while True:
            ...
            datareal_gx  = struct.unpack('<f',data_bytes[0:4])[0] # ax
            datareal_ax = struct.unpack('<f',data_bytes[4:8])[0] # ay
            datareal_theta  = struct.unpack('<f',data_bytes[8:12])[0] # az
            ...
            y[:,-1] = [datareal_gx,datareal_ax,datareal_theta]
            ...
            #writer.writerow(y[:,-1])
            writer.writerow([datareal_gx, datareal_ax, datareal_theta])
